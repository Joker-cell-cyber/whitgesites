...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  return (
    <div className={`relative rounded-2xl border ${isPopular ? 'border-purple-600 shadow-lg' : 'border-gray-200 dark:border-gray-800'} p-6 flex flex-col h-full`}>
      {isPopular && (
        <div className="absolute -top-5 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-3 py-1 rounded-full text-sm font-medium">
          Most Popular
        </div>
      )}
      ...
      <Link
        ...
        } text-base font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500`}
      >
        {ctaText}
      </Link>
    </div>
