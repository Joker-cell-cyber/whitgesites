    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-80 backdrop-blur-sm transition-opacity duration-300"
      ...
      <div 
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="text-xl font-bold text-vf-slate-200">{title}</h2>
          <button 
            className="p-2 rounded-full hover:bg-gray-800 transition-colors"
            onClick={handleClose}
            aria-label="Close">
            <XMarkIcon className="h-6 w-6 text-gray-400 hover:text-vf-slate-200" />
          </button>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-800 flex justify-end">
          <button 
            className="px-4 py-2 bg-gradient-to-r from-vid-red-600 to-vid-orange-500 text-vf-slate-900 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-vid-red-500 focus:ring-offset-2 focus:ring-offset-gray-900 button-glow"
            onClick={handleClose}>
            Close
          </button>
        </div>
