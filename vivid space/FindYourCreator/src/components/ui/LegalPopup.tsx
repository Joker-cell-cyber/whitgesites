...
const LegalPopup = ({ isOpen, onClose, title, children }: LegalPopupProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-80 backdrop-blur-sm transition-opacity duration-300"
      ...
      <div 
        className={`relative bg-gradient-to-b from-gray-900/90 to-gray-900/80 backdrop-blur-sm max-w-4xl w-full max-h-[90vh] rounded-xl shadow-2xl transition-all duration-300 transform overflow-hidden border border-vid-red-800/20 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}
