...
const FAQItem = ({ question, answer, isOpen, onToggle }: FAQItemProps) => {
  return (
    <div className="border-b border-ug-gray-200">
      <button
        onClick={onToggle}
        className="flex justify-between items-center w-full py-5 text-left"
      >
        <h3 className="text-lg font-medium text-ug-gray-900">{question}</h3>
        <ChevronDownIcon
          className={`h-5 w-5 text-ug-gray-500 transition-transform duration-300 ${
      ...
      >
        <p className="text-ug-gray-600">{answer}</p>
      </div>
    </div>
  );
};
...
export default function ContactFAQ() {
  ...
  return (
    <section className="py-20 relative overflow-hidden bg-ug-gray-50">
      <div className="absolute inset-0 z-0">
        <div className="absolute bottom-0 left-0 w-full h-64 bg-gradient-to-t from-ug-blue-50 to-transparent"></div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          <h2 className="text-3xl font-bold mb-4 text-ug-gray-900">Frequently Asked Questions</h2>
        ...
        <motion.div
          ...
          className="max-w-3xl mx-auto bg-white rounded-xl border border-ug-gray-200 overflow-hidden shadow-md"
        >
          {faqs.map((faq, index) => (
            <FAQItem
              key={index}
        ...
        >
          <p className="text-ug-gray-700 mb-6">
            Still have questions? We&apos;re here to help.
          </p>
          <a
            href="mailto:info@findyourcreator"
            className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-vivid-600 to-vivid-500 text-white rounded-lg font-medium button-glow"
          >
