...
const CookieConsent: React.FC = () => {
  ...
  // Afficher la bannière si l'utilisateur n'a pas donné son consentement
  useEffect(() => {
    if (!hasConsented) {
  ...
  // Mettre à jour les consentements locaux lorsque le contexte change
  useEffect(() => {
  ...
  // Pour tester : permet de réinitialiser les consentements
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          className="fixed bottom-0 left-0 right-0 z-50 bg-zinc-900 border-t border-zinc-800 shadow-lg"
                ...
                <div className="flex items-start justify-between p-4 bg-zinc-800/50 rounded-lg">
                  ...
                  <div className="flex items-center h-5">
                    <input
                      ...
                      disabled
                      className="w-4 h-4 accent-purple-600 bg-zinc-700 border-zinc-600 rounded focus:ring-purple-600"
                    />
                  </div>
                </div>
