...
export class ProductManager {
  ...
  public getSubscriptionId(plan: string, period: string): string | undefined {
    ...
    const productExact = this.products.find(p => {
      ...
      console.log(`DEBUG: Produit vérifié - ID: ${p.campaign_product_id}, Nom: ${p.product_name}, Plan match: ${planMatch}, Name match: ${nameMatch}`);
  ...
  public getTokenPackId(plan: string, tokenCount: number): string | undefined {
    ...
    const product = this.products.find(p => {
      ...
      console.log(`DEBUG: Token pack vérifié - ID: ${p.campaign_product_id}, Nom: ${p.product_name}, Plan: ${p.subscription_plan}, Plan match: ${planMatch}, Token match: ${tokenMatch}`);
  ...
  public buildCheckoutUrl(products: CheckoutProducts): string {
    const baseUrl = process.env.NEXT_PUBLIC_CHECKOUT_URL || 'https://payment-securise.adpulse-ai.com/checkout/IrP6mxrX8RU2oxFl69rHMV7SyBC4g58Py6drsdqyd6FjhzvVhRSFrbHVJTO4dvaxoEqwzXGXF1g4IiIKtRZFervvBCG';
