...
export async function getRedisClient() {
  ...
  try {
    ...
    const redisUrl = process.env.REDIS_URL || process.env.TOKENS_REDIS_URL;
    
    if (!redisUrl) {
      throw new Error('Variable d\'environnement REDIS_URL ou TOKENS_REDIS_URL non définie');
    }
    
    console.log('[Redis] Initialisation du client Redis...');
    
    // Créer un nouveau client Redis avec l'URL de la variable d'environnement
    ...
    try {
      ...
      console.log(`[Redis] Test de connexion: ${pong}`);
    } catch (pingError) {
      console.error('[Redis] Échec du test de connexion:', pingError);
    }
