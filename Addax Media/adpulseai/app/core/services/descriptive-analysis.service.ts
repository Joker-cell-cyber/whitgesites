...
export class DescriptiveAnalysisService {
  ...
  private detectAnomalies(data: AdvertisingData[], basicMetrics: any): Anomaly[] {
    // Pour chaque métrique importante, détecter les valeurs aberrantes
    const anomalies: Anomaly[] = [];
    
    // Regrouper les données par jour
    const groupedByDate = this.groupDataByDate(data);
    
    // Métriques à vérifier pour les anomalies
    const metricsToCheck = [
      { name: 'ctr', label: 'Taux de clics', thresholdMultiplier: 2 },
      { name: 'cpc', label: 'Coût par clic', thresholdMultiplier: 2 },
      { name: 'cpa', label: 'Coût par acquisition', thresholdMultiplier: 2.5 },
      { name: 'conversionRate', label: 'Taux de conversion', thresholdMultiplier: 2 }
    ];
    ...
    for (const metric of metricsToCheck) {
      ...
      for (const [date, dailyData] of Object.entries(groupedByDate)) {
        // Calculer la valeur agrégée pour ce jour
        let aggregatedValue: number;
        
        if (['ctr', 'cpc', 'cpa', 'conversionRate'].includes(metric.name)) {
          // Pour les taux et coûts, calculer la moyenne pondérée
          let totalValue = 0;
          let totalWeight = 0;
          ...
          for (const row of dailyData) {
            ...
            if (metric.name === 'ctr') {
              ...
              weight = row.clicks || 0;
            } else if (metric.name === 'cpa' || metric.name === 'conversionRate') {
              weight = row.conversions || 0;
            }
          ...
          aggregatedValue = totalWeight > 0 ? totalValue / totalWeight : 0;
        } else {
          // Pour les valeurs absolues, calculer la somme
          aggregatedValue = dailyData.reduce((sum, row) => {
            const value = row[metric.name as keyof AdvertisingData];
            return sum + (typeof value === 'number' ? value : 0);
          }, 0);
        }
        ...
        if (aggregatedValue > upperThreshold || aggregatedValue < lowerThreshold) {
          ...
          anomalies.push({
            ...
            description: `${metric.label} ${isHigh ? 'anormalement élevé' : 'anormalement bas'} (${aggregatedValue.toFixed(2)}) par rapport à la moyenne (${expectedValue.toFixed(2)})`,
            impact: metric.name === 'cpa' ? (isHigh ? 'negative' : 'positive') : (isHigh ? 'positive' : 'negative'),
            ...
            expectedValue,
            deviation,
            suggestedAction: this.generateAnomalyAction(metric.name, isHigh)
          });
        }
      }
    }
   ...
   */
  private generateAnomalyAction(metric: string, isHigh: boolean): string {
    switch (metric) {
      case 'ctr':
        return isHigh 
          ? 'Analyser les audiences et créatifs de cette journée pour comprendre ce qui a fonctionné exceptionnellement bien'
          : 'Vérifier la pertinence des créatifs et le ciblage pour cette journée';
      
      case 'cpc':
        return isHigh 
          ? 'Examiner les enchères et la concurrence sur cette période, potentiellement ajuster le ciblage'
          : 'Analyser les facteurs ayant conduit à ce CPC bas pour reproduire ce succès';
      
      case 'cpa':
        return isHigh 
          ? 'Vérifier la qualité du trafic et le parcours de conversion pour identifier les obstacles'
          : 'Étudier les facteurs de succès de cette journée pour optimiser les autres campagnes';
      
      default:
        return 'Analyser en détail cette anomalie pour en comprendre les causes et les implications';
    }
  }
