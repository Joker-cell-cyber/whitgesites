...
export default function ChatContainer({ conversation, onPersonalityChange }: ChatContainerProps) {
  ...
  if (!conversation) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 bg-gradient-to-b from-pink-50/80 to-white backdrop-blur-sm">
        <div className="mb-6">
          <Sparkles className="h-16 w-16 text-pink-400/80 mx-auto" />
        </div>
        
        <h2 className="text-2xl font-bold text-gray-800 text-center mb-2">
          Bienvenue chez CharmAI
        </h2>
        
        <p className="text-gray-600 text-center max-w-md mx-auto mb-8">
          Je suis votre experte en séduction et relations amoureuses. Posez-moi toutes vos questions sur l'amour, le dating et les relations.
        </p>
        
        <div className="grid grid-cols-2 gap-6 max-w-2xl mx-auto w-full">
          <div className="bg-gradient-to-r from-pink-50 to-rose-50 rounded-xl border border-pink-200 p-5 hover:border-pink-300 transition-all shadow-sm hover:shadow-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-2 flex items-center gap-2">
              <User className="h-5 w-5 text-pink-500" />
              <span>Premiers pas</span>
            </h3>
            <p className="text-sm text-gray-600 mb-3">Comment initier une conversation intéressante ? Comment faire bonne impression au premier rendez-vous ?</p>
          </div>
          
          <div className="bg-gradient-to-r from-pink-50 to-rose-50 rounded-xl border border-pink-200 p-5 hover:border-pink-300 transition-all shadow-sm hover:shadow-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-2 flex items-center gap-2">
              <User className="h-5 w-5 text-rose-400" />
              <span>Signes d'intérêt</span>
            </h3>
            <p className="text-sm text-gray-600 mb-3">Comment savoir si quelqu'un est vraiment intéressé ? Quels sont les signes qui ne trompent pas ?</p>
          </div>
          
          <div className="bg-gradient-to-r from-pink-50 to-rose-50 rounded-xl border border-pink-200 p-5 hover:border-pink-300 transition-all shadow-sm hover:shadow-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-2 flex items-center gap-2">
              <RefreshCw className="h-5 w-5 text-pink-400" />
              <span>Relations durables</span>
            </h3>
            <p className="text-sm text-gray-600 mb-3">Comment entretenir la flamme dans une relation ? Quelles sont les clés d'une relation équilibrée ?</p>
          </div>
          
          <div className="bg-gradient-to-r from-pink-50 to-rose-50 rounded-xl border border-pink-200 p-5 hover:border-pink-300 transition-all shadow-sm hover:shadow-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-2 flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-rose-500" />
              <span>Séduction avancée</span>
            </h3>
            <p className="text-sm text-gray-600 mb-3">Comment créer une alchimie irrésistible ? Quelles techniques subtiles pour attirer l'attention de votre crush ?</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-white to-pink-50/60 backdrop-blur-sm">
      ...
      <div className="p-4 border-b border-pink-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-4 flex items-center gap-1 px-2 py-0.5 rounded-full bg-pink-100 text-xs">
            <Sparkles className="h-3 w-3 text-pink-500" />
            <span className="font-medium text-pink-600">{displayTokens} tokens</span>
          </div>
        </div>
      ...
      {/* Zone des messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[url('/patterns/hearts-pattern.svg')] bg-repeat bg-opacity-5">
        {messages.length === 0 ? (
          ...
          <>
            ...
            <AnimatePresence>
              {isTyping && (
                ...
                >
                  <div className="max-w-[80%] rounded-2xl p-4 bg-white border border-pink-100 shadow-sm">
      ...
      <AnimatePresence>
        {showTokenUsage && (
          <motion.div
            ...
            animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            className="bg-pink-50 border-t border-pink-100 p-3 text-pink-600 text-sm"
          >
      ...
      <div className="p-4 border-t border-pink-100 bg-white">
        <form onSubmit={handleSendMessage} className="relative">
          <textarea
            ...
            className="w-full py-3 px-4 pr-12 bg-white text-gray-800 border border-pink-100 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-pink-200 max-h-32"
            rows={1}
          ...
          <button
            ...
            className="absolute right-3 bottom-3 p-1.5 rounded-full bg-gradient-to-r from-pink-500 to-fuchsia-500 text-white disabled:opacity-50 disabled:cursor-not-allowed"
          >
