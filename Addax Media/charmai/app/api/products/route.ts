...
import { fallbackProductData, ProductData, PLAN_NAME_MAPPING, CSV_FILENAME } from '@/lib/checkout-products';
...
export async function GET() {
  try {
    ...
    try {
        ...
        if (productName.toLowerCase().includes('token') || productName.toLowerCase().includes('jeton')) {
          ...
          if (tokenMatch) {
            ...
            if (planName && planName !== '') {
              ...
              for (const [englishName, frenchName] of Object.entries(PLAN_NAME_MAPPING)) {
                if (planName.toLowerCase() === englishName.toLowerCase()) {
                  matchedPlan = frenchName;
                  break;
                }
              }
          ...
          let matchedPlan = null;
          for (const [englishName, frenchName] of Object.entries(PLAN_NAME_MAPPING)) {
            if (productName.toLowerCase().includes(englishName.toLowerCase())) {
              matchedPlan = frenchName;
              break;
            }
          }
      ...
      console.log('Abonnements:', JSON.stringify(data.subscriptionProducts, null, 2));
      console.log('Tokens à la carte:', JSON.stringify(data.tokenProducts, null, 2));
      console.log('Tokens par plan:', JSON.stringify(data.subscriptionTokenProducts, null, 2));
