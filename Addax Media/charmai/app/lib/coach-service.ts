...
export async function generateCoachResponse(
  ...
  try {
    ...
    // Sinon, nous utilisons le dernier message de l'assistant
    const lastMessage = messages[messages.length - 1];
    
    if (lastMessage && lastMessage.sender === 'user') {
      logWithTimestamp(`Dernier message est de l'utilisateur: "${lastMessage.content ? lastMessage.content.substring(0, 50) + '...' : 'Contenu vide'}"`);
      
      // Assurer que le dernier message utilisateur est dans le contexte
      const lastUserMessage: ChatCompletionUserMessageParam = {
        role: "user",
      ...
      if (contextLastMessage.role !== 'user' || contextLastMessage.content !== lastMessage.content) {
        conversationContext.messages.push(lastUserMessage);
        logWithTimestamp(`Ajout du message utilisateur au contexte: "${lastMessage.content ? lastMessage.content.substring(0, 50) + '...' : 'Contenu vide'}"`);
      } else {
      ...
      // Appel à l'API OpenAI avec le contexte existant
...
/**
 * Génère un message d'accueil pour une nouvelle conversation
 */
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: "Génère un message d'accueil chaleureux, amical et engageant pour commencer la conversation. Reste dans ton personnage de coach en séduction." }
      ],
      temperature: 0.9,
      max_tokens: 300
    });
