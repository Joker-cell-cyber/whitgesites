  ...
  return (
    <AuthGuard>
      <div className="min-h-screen bg-white relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-pink-50 via-white to-rose-50 opacity-80 z-0"></div>
        ...
        <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-pink-100">
          <div className="flex items-center justify-between px-4 h-16">
            ...
            <Link href="/chat" className="flex items-center gap-2">
              <span className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-rose-500">CharmAI</span>
            </Link>

            {/* Tokens et Menu */}
            <div className="flex items-center gap-4">
              {/* Affichage des tokens */}
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-pink-50 border border-pink-100">
                <Heart className="h-4 w-4 text-pink-500" />
                <span className="text-sm font-medium text-gray-800">{displayTokens}</span>
                <Sparkles className="h-3 w-3 text-pink-400 opacity-80" />
              </div>

              {/* Menu mobile */}
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="lg:hidden p-2 rounded-lg bg-white/90 backdrop-blur-sm border border-pink-100"
                aria-label="Menu"
              >
              ...
              <div className="hidden lg:flex items-center gap-2">
                ...
                <Link
                  href="/subscriptions"
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-pink-50 transition-colors"
                >
        ...
        <div className={`fixed inset-0 z-40 bg-white/95 backdrop-blur-md lg:hidden transform transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex flex-col h-full pt-16">
            <nav className="flex-1 p-4 space-y-2">
              ...
              <Link
                href="/subscriptions"
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:text-gray-900 hover:bg-pink-50 transition-colors"
                onClick={() => setIsMobileMenuOpen(false)}
              >
              ...
              <button
                ...
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:text-gray-900 hover:bg-pink-50 transition-colors w-full"
              >
