  ...
  if (showProfileForm) {
    return (
      <div className="container mx-auto py-6 max-w-3xl">
        <h1 className="text-2xl font-bold text-gradient-primary mb-6">Configurez votre Coach Personnalisé</h1>
        ...
        <div className="glass-card border-gradient p-6">
          <form onSubmit={handleProfileSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-white mb-1">Âge</label>
                <Input
                  type="number"
                  name="age"
                  value={formData.age}
                  ...
                  className="border-red-500/20 bg-black/30 text-white placeholder:text-gray-500 focus:border-red-500"
                  required
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={handleInputChange}
                  className="w-full h-10 rounded-md border border-red-500/20 bg-black/30 text-white px-3 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500"
                  required
                >
              ...
              <div>
                <label className="block text-sm font-medium text-white mb-1">Niveau d'activité</label>
                <select
                  name="activityLevel"
              ...
              <div>
                <label className="block text-sm font-medium text-white mb-1">Niveau de fitness</label>
                <select
                  name="fitnessLevel"
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">Objectifs fitness (séparés par des virgules)</label>
              <Input
                type="text"
                name="fitnessGoals"
                value={formData.fitnessGoals}
                onChange={handleInputChange}
                placeholder="Ex: perte de poids, prise de muscle, endurance..."
                className="border-red-500/20 bg-black/30 text-white placeholder:text-gray-500 focus:border-red-500"
                required
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">Préférences alimentaires (séparées par des virgules)</label>
              <Input
                type="text"
                name="dietaryPreferences"
                value={formData.dietaryPreferences}
                onChange={handleInputChange}
                placeholder="Ex: végétarien, sans gluten, cétogène..."
                className="border-red-500/20 bg-black/30 text-white placeholder:text-gray-500 focus:border-red-500"
              />
            </div>
            
            <div>
              <label className="block text-sm font-medium text-white mb-1">Conditions médicales (séparées par des virgules, optionnel)</label>
              <Textarea
                name="medicalConditions"
                value={formData.medicalConditions}
                onChange={handleInputChange}
                placeholder="Ex: douleurs au dos, problèmes articulaires, diabète..."
                className="border-red-500/20 bg-black/30 text-white placeholder:text-gray-500 focus:border-red-500"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-white mb-1">Fréquence d'entraînement (jours/semaine)</label>
                <Input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-white mb-1">Durée d'entraînement (minutes)</label>
                <Input
                  type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">Expérience précédente (optionnel)</label>
              <Textarea
                name="previousExperience"
                value={formData.previousExperience}
                onChange={handleInputChange}
                placeholder="Décrivez brièvement votre expérience passée en fitness..."
                className="border-red-500/20 bg-black/30 text-white placeholder:text-gray-500 focus:border-red-500"
              />
            </div>
            ...
            <div className="flex justify-center pt-4">
              <Button 
                type="submit" 
                className="px-8 py-2 bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-700 hover:to-orange-600 text-white font-medium rounded-lg shadow-lg"
              >
                Commencer avec mon coach IA
              </Button>
            </div>
          </form>
        </div>
