...
export default function AuthStatus({ className, isMobile }: AuthStatusProps) {
  ...
  return (
    <div className={className}>
      {isAuthenticated ? (
        ...
        <>
          {/* Bouton de désabonnement rendu plus visible avec des couleurs orange */}
          <Link href="/unsubscribe" className={`
            ${isMobile ? 'mb-2' : ''}
            px-4 sm:px-5 py-2 sm:py-2.5 text-sm sm:text-base font-medium
            bg-[#FFEDD5] text-[#F97316] border border-[#F97316]/30
          ...
          <Link href="/login" className="relative group overflow-hidden rounded-xl cursor-pointer active:scale-95 w-full md:w-auto">
            <div className="absolute inset-0 bg-gradient-to-r from-[#F97316] to-[#FB923C] rounded-xl"></div>
            <div className="absolute inset-0 bg-black/10 rounded-xl m-[1px] group-hover:m-[2px] transition-all duration-200"></div>
            <span className="relative z-10 px-4 sm:px-6 py-2 sm:py-2.5 text-sm sm:text-base text-white rounded-xl bg-transparent transition-all duration-200 block text-center">
              Connexion
            </span>
