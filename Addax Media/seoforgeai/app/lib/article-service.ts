...
export async function getArticles(): Promise<Article[]> {
  try {
    // Tenter d'obtenir les articles authentifiés
    const articles = await getArticlesFromAuth();
    
    // Si nous avons des articles, les retourner
    if (articles && articles.length > 0) {
      return articles;
    }
    
    // En production, si aucun article n'est trouvé, retourner un tableau vide
    if (process.env.NODE_ENV === 'production') {
      return [];
    }
    
    // En développement seulement, retourner les articles de démo si aucun article n'est trouvé
    return process.env.NODE_ENV === 'development' ? demoArticles : [];
  } catch (error) {
    console.error('Error fetching articles:', error);
    
    // En production, retourner un tableau vide en cas d'erreur
    if (process.env.NODE_ENV === 'production') {
      return [];
    }
    
    // En développement seulement, retourner les articles de démo en cas d'erreur
    return process.env.NODE_ENV === 'development' ? demoArticles : [];
  }
}
