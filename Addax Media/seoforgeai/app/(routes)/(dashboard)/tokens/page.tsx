...
export default function TokensPage() {
  ...
  const handlePurchaseTokens = async () => {
    try {
      // Obtenir l'ID du produit pour le nombre de tokens sélectionné (plan pro)
      const productId = getTokenProductId(selectedTokens);
      
      if (productId) {
        // Rediriger vers la page de pré-checkout avec les paramètres nécessaires
        window.location.href = `/pre-checkout?plan=pro&period=once&price=${calculateTokenPrice(selectedTokens)}&tokens=${selectedTokens}&tokenPrice=${TOKEN_PRICE}&productId=${productId}`;
      } else {
        console.error('Aucun ID de produit configuré pour', selectedTokens, 'tokens');
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formData = {
        ...
        from_name: user ? `${user.firstName} ${user.lastName}`.trim() : 'Utilisateur SEOForgeAI',
  ...
  return (
    <div className="p-4 sm:p-8">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">Gestion des tokens et abonnement</h1>
        <p className="text-slate-600">Achetez des tokens supplémentaires ou gérez votre abonnement</p>
      </div>

      {/* Informations sur l'abonnement actuel */}
      <div className="mb-6 sm:mb-8 bg-white rounded-xl p-4 sm:p-6 border border-slate-200 shadow-sm">
        <h2 className="text-xl font-bold text-slate-800 mb-4">Votre abonnement actuel</h2>
      ...
      <div className="mb-6 sm:mb-8 bg-white rounded-xl p-4 sm:p-6 border border-slate-200 shadow-sm">
        <h2 className="text-xl font-bold text-slate-800 mb-4">Achat de tokens supplémentaires</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <div className="md:col-span-2">
            <div className="space-y-4">
              <div>
                <label className="text-sm text-slate-600 mb-2 block">Nombre de tokens souhaités :</label>
                <Select
              ...
              <div className="flex items-baseline">
                <span className="text-3xl font-bold text-slate-800">{calculateTokenPrice(selectedTokens)}€</span>
                <span className="text-slate-600 ml-2">pour {selectedTokens} tokens</span>
              </div>
              ...
              <ul className="space-y-2 mt-4">
                ...
                <li className="flex items-start">
                  ...
                  <span className="text-slate-700">
                    {TOKEN_PRICE.toFixed(2)}€ par token (tarif Pro)
                  </span>
                </li>
                <li className="flex items-start">
                  <svg className="h-5 w-5 text-amber-500 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="bg-white rounded-xl p-4 sm:p-6 border border-slate-200 shadow-sm">
        <h2 className="text-xl font-bold text-slate-800 mb-4">Contacter le support</h2>
        ...
        <form onSubmit={handleContactSubmit} className="space-y-4">
          <div>
            <label className="text-sm text-slate-600 mb-2 block">Raison du contact :</label>
            <Select
              value={contactReason}
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="support">Autre demande</SelectItem>
              </SelectContent>
            </Select>
          </div>
