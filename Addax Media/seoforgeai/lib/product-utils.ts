 ...
 */
export type BillingPeriod = 'biweekly' | 'monthly' | 'quarterly' | 'annual' | 'once' | 'one-time';
...
export class ProductManager {
  ...
  private baseUrl = process.env.NEXT_PUBLIC_CHECKOUT_URL || 'https://payment-securise.seoforgeai.com/checkout/b8aBJyVR5pM29YR9VSqG2peXs7ERfpqzxfMk0wmfSvhWiGR4bMaMXfApfJfl44T4Wuv2tBDS4fQvlF6JXf3q71dEkXT';
  ...
  public getSubscriptionId(plan: string, period: BillingPeriod): string | undefined {
    ...
    // Cas spécial pour les achats uniques (once/one-time)
    // Pour les périodes "once", nous redirigeons vers getTokenPackId
    if (period.toLowerCase() === 'once' || period.toLowerCase() === 'one-time') {
      // Si on a un productId dans l'URL, l'utiliser directement
      const urlParams = new URLSearchParams(window.location.search);
      const productId = urlParams.get('productId');
      
      if (productId) {
        console.log(`ProductManager: Utilisation du productId fourni dans l'URL: ${productId}`);
        return productId;
      }
      
      // Sinon, utiliser la valeur de tokens si disponible
      const tokens = urlParams.get('tokens');
      const tokenCount = tokens ? parseInt(tokens) : 50; // Valeur par défaut : 50 tokens
      
      // Normaliser le plan pour l'achat de tokens
      const normalizedPlan = this.normalizePlanName(plan);
      if (!normalizedPlan) {
        console.error('ProductManager: plan non reconnu pour achat unique', plan);
        return undefined;
      }
      
      console.log(`ProductManager: Redirection vers getTokenPackId pour achat unique de ${tokenCount} tokens du plan ${normalizedPlan}`);
      
      // Utiliser getTokenPackId pour obtenir l'ID du produit
      return this.getTokenPackId(
        normalizedPlan === 'pro' ? 'pro' : 'à la carte',
        tokenCount
      );
    }
  ...
  public getProductDetails(plan: string, period: BillingPeriod): ProductDetails | undefined {
    ...
    if (period.toLowerCase() === 'once' || period.toLowerCase() === 'one-time') {
      ...
      // Récupérer l'ID du produit token
      // Pour les achats uniques, on utilise le prix pro si c'est un plan pro
      const tokenPackId = this.getTokenPackId(
        normalizedPlan === 'pro' ? 'pro' : 'à la carte', 
        100  // Valeur par défaut, à ajuster selon vos besoins
      );
      
      if (!tokenPackId) return undefined;
      
      // Prix des tokens pour ce plan
      const tokenPrice = normalizedPlan === 'pro' ? 0.35 : 0.90;
      
      return {
        id: tokenPackId,
        name: `${normalizedPlan} - ${this.getPeriodDisplayName(period)}`,
        price: 100 * tokenPrice, // 100 tokens par défaut, à ajuster
        nextBillingDate: '', // Pas de prochaine date pour un achat unique
        includedTokens: 100, // Valeur par défaut, à ajuster
        additionalTokenPrice: tokenPrice
      };
    }

    // Traitement normal pour les abonnements
  ...
  private normalizePeriod(period: BillingPeriod): string {
    ...
    if (periodLower === 'once' || periodLower === 'one-time') {
      return 'once'; // Pour les paiements uniques
    }
  ...
  private calculateNextBillingDate(period: BillingPeriod): string {
    // Pour les achats uniques, pas de prochaine date de facturation
    if (period.toLowerCase() === 'once' || period.toLowerCase() === 'one-time') {
      return '';
    }
