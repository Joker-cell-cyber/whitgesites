  ...
  const handlePurchaseTokens = async () => {
    try {
      ...
      if (checkoutLink) {
        ...
        window.location.href = `/pre-checkout?plan=À la carte&period=once&price=${calculateTokenPrice(selectedTokens)}&tokens=${selectedTokens}&tokenPrice=${enterpriseTokenPrice.toFixed(2)}`;
      } else {
          ...
          <div className="md:col-span-2">
            <div className="space-y-4">
              ...
              <div className="flex items-baseline">
                <span className="text-3xl font-bold text-[#14304D]">{calculateTokenPrice(selectedTokens)}€</span>
                <span className="text-[#14304D]/70 ml-2">pour {selectedTokens} tokens</span>
              </div>
      ...
      <div className="bg-black/40 backdrop-blur-xl rounded-xl p-4 sm:p-6 border border-purple-500/20">
        ...
        <form onSubmit={handleContactSubmit} className="space-y-4">
          ...
          <input type="hidden" name="subject" value={`Demande de ${contactReason === 'change_plan' ? 'changement de plan' : 
                contactReason === 'change_billing' ? 'modification de facturation' : 
                'support'} - ${user?.email || 'Utilisateur'}`} />
          <input type="hidden" name="from_name" value={user ? `${user.firstName} ${user.lastName}`.trim() : 'Utilisateur OneirICrafts'} />
