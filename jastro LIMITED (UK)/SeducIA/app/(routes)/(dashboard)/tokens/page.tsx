...
export default function TokensPage() {
  ...
  const handlePurchaseTokens = async () => {
    try {
      ...
      if (checkoutLink) {
        ...
        window.location.href = `/pre-checkout?plan=À la carte&period=once&price=${calculateTokenPrice(selectedTokens)}&tokens=${selectedTokens}&tokenPrice=${enterpriseTokenPrice}`;
      } else {
  ...
  return (
    <div className="p-4 sm:p-8 relative bg-[#FAF8F4]">
      ...
      {/* Background pattern */}
      <div className="fixed inset-0 bg-[url('/subtle-pattern.svg')] bg-repeat opacity-[0.03] pointer-events-none"></div>
      
      <div className="mb-6 sm:mb-8 relative z-10">
        <h1 className="text-2xl sm:text-3xl font-bold text-[#45301C] mb-2">Acheter des tokens</h1>
        <p className="text-[#45301C]/80">Augmentez votre solde de tokens pour continuer à profiter de nos services premium.</p>
      </div>

      {/* Achat de tokens supplémentaires */}
      <div className="mb-6 sm:mb-8 bg-white rounded-xl p-4 sm:p-6 border border-[#F1EDE8] shadow-md relative z-10">
        <h2 className="text-xl font-bold text-[#45301C] mb-4">Achat de tokens supplémentaires</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <div className="md:col-span-2">
            <div className="space-y-4">
              <div>
                <label className="text-sm text-[#45301C]/70 mb-2 block">Nombre de tokens souhaités :</label>
                <Select
                  value={selectedTokens.toString()}
                  onValueChange={(value) => setSelectedTokens(parseInt(value))}
                >
                  <SelectTrigger className="w-full bg-[#FAF8F4] border-[#F1EDE8] focus:border-[#E46D4B] focus:ring-[#E46D4B]/20 text-[#45301C]">
                    <SelectValue placeholder="Sélectionnez un nombre de tokens" />
                  </SelectTrigger>
                  <SelectContent className="max-h-60 bg-white border-[#F1EDE8]">
                    {availableTokenOptions.map((option) => (
                      <SelectItem key={option} value={option.toString()} className="text-[#45301C] hover:bg-[#FAF8F4]">
                        {option} tokens
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="flex items-baseline">
                <span className="text-3xl font-bold text-[#E46D4B]">{(selectedTokens * enterpriseTokenPrice).toFixed(2)}€</span>
                <span className="text-[#45301C]/70 ml-2">pour {selectedTokens} tokens</span>
              </div>
              <ul className="space-y-2 mt-4">
                <li className="flex items-start">
                  <svg className="h-5 w-5 text-[#E46D4B] mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-[#45301C]">
          ...
          <div className="flex flex-col justify-end mt-4 md:mt-0">
            <Button 
              ...
              className="w-full bg-[#E46D4B] hover:bg-[#D45C3A] shadow-md text-white"
            >
              Acheter des tokens
            </Button>
            {purchaseSuccess && (
      ...
      <div className="bg-white rounded-xl p-4 sm:p-6 border border-[#F1EDE8] shadow-md relative z-10">
        <h2 className="text-xl font-bold text-[#45301C] mb-4">Contacter le support</h2>
        <p className="text-[#45301C]/80 mb-6">
          Pour changer de plan d'abonnement ou modifier votre période de facturation, veuillez contacter notre équipe.
        </p>
        
        <form onSubmit={handleContactSubmit} className="space-y-4">
          <div>
            <label className="text-sm text-[#45301C]/70 mb-2 block">Raison du contact :</label>
            <Select
              value={contactReason}
            ...
            >
              ...
              <SelectContent className="bg-white border-[#F1EDE8]">
                <SelectItem value="change_plan" className="text-[#45301C] hover:bg-[#FAF8F4]">Changer de plan d'abonnement</SelectItem>
                <SelectItem value="change_billing" className="text-[#45301C] hover:bg-[#FAF8F4]">Modifier la période de facturation</SelectItem>
                <SelectItem value="other" className="text-[#45301C] hover:bg-[#FAF8F4]">Autre demande</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div>
            <label className="text-sm text-[#45301C]/70 mb-2 block">Votre message :</label>
            <Textarea
              value={contactMessage}
              onChange={(e) => setContactMessage(e.target.value)}
              placeholder="Décrivez votre demande en détail..."
              className="w-full bg-[#FAF8F4] border-[#F1EDE8] focus:border-[#E46D4B] focus:ring-[#E46D4B]/20 rounded-lg p-3 text-[#45301C] h-32"
              required
            />
          </div>
          ...
          <div className="flex justify-center sm:justify-start">
            <Button 
              type="submit"
              className="w-full sm:w-auto bg-[#E46D4B] hover:bg-[#D45C3A] shadow-md text-white"
              disabled={submitting}
            >
              {submitting ? (
                <>
