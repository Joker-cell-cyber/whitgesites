...
export interface Transaction {
  ...
  amount: number;
  description: string;
  contentType?: 'ARTICLE' | 'PRODUCT_DESCRIPTION' | 'FACEBOOK_ANALYSIS' | 'FACEBOOK_RECOMMENDATIONS';
...
export async function recordGeneration(
  ...
  tokenCost: number,
  contentType: 'ARTICLE' | 'PRODUCT_DESCRIPTION' | 'FACEBOOK_ANALYSIS' | 'FACEBOOK_RECOMMENDATIONS',
  description: string = contentType === 'PRODUCT_DESCRIPTION' ? 'Génération description produit' : 
               contentType === 'FACEBOOK_ANALYSIS' ? 'Analyse Facebook Ads' :
               contentType === 'FACEBOOK_RECOMMENDATIONS' ? 'Recommandations Facebook Ads' : 'Génération article'
  ...
  try {
    ...
    // Note: Pas de compteurs spécifiques pour les analyses Facebook
    ...
    console.log(`[KVStore] ${tokenCost} tokens déduits pour ${userId}. Type: ${contentType}. Nouveau solde: ${stats.tokenBalance}`);
    
    return {
      success: true,
      message: `${tokenCost} tokens déduits pour ${description}. Nouveau solde: ${stats.tokenBalance}`,
      stats
    };
  } catch (error) {
    console.error(`[KVStore] Erreur lors de l'enregistrement pour ${userId}:`, error);
    return {
      success: false,
