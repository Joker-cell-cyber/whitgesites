  ...
  if (hasError) {
    return (
      <div className="p-8 text-center">
        <h1 className="text-3xl font-bold text-[#2A303D] mb-4">Une erreur s'est produite</h1>
        <p className="text-gray-600 mb-6">Impossible de charger les données du tableau de bord.</p>
        <Button 
          onClick={() => window.location.reload()} 
          className="bg-gradient-to-r from-[#A590DC] to-[#8A72CA] hover:from-[#9A82D0] hover:to-[#7D62BD]"
        >
          Réessayer
        </Button>
      </div>
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {/* Tokens restants */}
        <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
          <h3 className="text-gray-600 text-sm mb-2">Tokens restants</h3>
          {isLoading || !stats ? (
            <div className="h-8 w-24 bg-gray-200 rounded animate-pulse"></div>
          ) : (
            <p className="text-3xl font-bold text-[#2A303D]">{stats.tokensRemaining}</p>
          )}
          <div className="mt-4 w-full h-2 bg-gray-100 rounded-full overflow-hidden">
            {!isLoading && stats && (
              <div
                className="h-full bg-gradient-to-r from-[#A590DC] to-[#E2D9F3]"
                style={{
        ...
        {/* Tokens utilisés */}
        <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
          <h3 className="text-gray-600 text-sm mb-2">Tokens utilisés</h3>
          {isLoading || !stats ? (
            <div className="h-8 w-24 bg-gray-200 rounded animate-pulse"></div>
          ) : (
            <p className="text-3xl font-bold text-[#2A303D]">{stats.totalSpent}</p>
          )}
        ...
        {/* Générations totales */}
        <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
          <h3 className="text-gray-600 text-sm mb-2">Générations totales</h3>
          {isLoading || !stats ? (
            <div className="h-8 w-24 bg-gray-200 rounded animate-pulse"></div>
          ) : (
            <p className="text-3xl font-bold text-[#2A303D]">
              {
        ...
        {/* Prochain achat */}
        <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
          <h3 className="text-gray-600 text-sm mb-2">Prochain achat</h3>
          {isLoading || !stats ? (
            <div className="h-8 w-24 bg-gray-200 rounded animate-pulse"></div>
          ) : (
            <p className="text-3xl font-bold text-[#2A303D]">Maintenant</p>
          )}
          <p className="text-gray-500 text-xs mt-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
        <div className="p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
          <h4 className="text-gray-600 text-xs">Articles</h4>
          <p className="text-2xl font-semibold mt-1">
            {isLoading || !stats ? (
              <span className="h-8 w-12 bg-gray-200 rounded animate-pulse block"></span>
            ) : (
        ...
        <div className="p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
          <h4 className="text-gray-600 text-xs">Messages coach</h4>
          <p className="text-2xl font-semibold mt-1">
            {isLoading || !stats ? (
              <span className="h-8 w-12 bg-gray-200 rounded animate-pulse block"></span>
            ) : (
        ...
        <div className="p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
          <h4 className="text-gray-600 text-xs">Programmes</h4>
          <p className="text-2xl font-semibold mt-1">
            {isLoading || !stats ? (
              <span className="h-8 w-12 bg-gray-200 rounded animate-pulse block"></span>
            ) : (
        ...
        <div className="p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
          <h4 className="text-gray-600 text-xs">Plans nutrition</h4>
          <p className="text-2xl font-semibold mt-1">
            {isLoading || !stats ? (
              <span className="h-8 w-12 bg-gray-200 rounded animate-pulse block"></span>
            ) : (
      ...
      <div className="mt-6 bg-white rounded-xl p-6 border border-gray-100 shadow-sm space-y-3">
        {/* Actions rapides et achats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {/* Carte pour le plan */}
          <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
            <h3 className="text-gray-600 text-sm mb-2">Plan Actuel</h3>
            <p className="text-2xl font-bold text-[#2A303D] mb-2">
              {user?.subscriptionPlan || 'Freemium'}
            </p>
            <Button className="w-full bg-gradient-to-r from-[#A590DC] to-[#8A72CA] hover:from-[#9A82D0] hover:to-[#7D62BD] text-white" asChild>
          ...
          <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
            <h3 className="text-gray-600 text-sm mb-2">Recharge de tokens</h3>
          ...
          <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
            <h3 className="text-gray-600 text-sm mb-2">Coach IA</h3>
            <div className="text-gray-500 mb-4">
        ...
        <div className="bg-white rounded-xl p-6 border border-[#E2D9F3] shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-[#2A303D]">Informations sur votre plan</h2>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="text-lg font-medium text-[#2A303D] mb-2">Plan {subscriptionPlan}</h3>
                <p className="text-gray-500 mb-4">
                  Votre plan vous donne accès à toutes les fonctionnalités premium de notre plateforme.
                </p>
                <p className="text-gray-500">
                  <span className="text-[#A590DC]">+300 tokens</span> sont ajoutés automatiquement à votre compte chaque mois.
                </p>
              </div>
              <div className="bg-gradient-to-r from-[#A590DC]/10 to-[#E2D9F3]/10 rounded-lg p-4 border border-[#E2D9F3]">
                <h4 className="text-[#2A303D] font-medium mb-2">Avantages de votre plan</h4>
                <p className="text-gray-500 mb-2">
                  Tokens mensuels: <span className="text-[#2A303D]">300</span>
                </p>
                <p className="text-gray-500">
      ...
      <div className="mt-6">
        <h2 className="text-xl font-bold text-[#2A303D] mb-4">Dernière activité</h2>
        
        {/* Liste des transactions récentes */}
        <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
          <h3 className="text-lg font-medium text-[#2A303D] mb-4">Transactions récentes</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-gray-500">Achat de tokens</span>
                </div>
              ...
              <div className="flex justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-gray-500">Génération d'article</span>
                </div>
              ...
              <div className="flex justify-between">
                <div className="flex items-center space-x-2">
                  <div className="w-2 h-2 rounded-full bg-red-500"></div>
                  <span className="text-gray-500">Conversation coach IA</span>
                </div>
          ...
          <div className="mt-4">
            <Button variant="outline" className="w-full border-gray-200 text-gray-500 hover:bg-gray-200">
