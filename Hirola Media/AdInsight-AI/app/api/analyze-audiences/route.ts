...
export async function POST(req: NextRequest) {
  try {
    ...
    // Coût en tokens pour l'analyse des audiences
    const tokenCost = 1;
    ...
    const deductionResult = await recordGeneration(
      ...
      tokenCost,
      'FACEBOOK_ANALYSIS',
      'Analyse des audiences Facebook'
    );
    ...
    try {
      const audiencesAnalysis = await OpenAIService.analyzeAudiences(data, basicMetrics);
      ...
      return NextResponse.json(
        { 
          success: false,
          error: 'Erreur lors de l\'analyse des audiences',
