...
export default function SubscriptionsPage() {
  ...
  return (
    <div className="p-8 relative bg-gradient-to-b from-white to-yfc-cream-50">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4 font-serif">Désabonnement</h1>
      <p className="text-gray-600 mb-6">Gérez vos abonnements et options de renouvellement ci-dessous.</p>
      
      {isLoading ? (
        <div className="flex justify-center items-center h-64">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-yfc-gold-500"></div>
        </div>
      ) : subscriptions.length === 0 ? (
        <div className="bg-white rounded-xl p-6 border border-yfc-cream-200 shadow-md relative z-10">
          <p className="text-gray-600 text-center">Vous n'avez aucun abonnement actif.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {subscriptions.map(subscription => (
            <div key={subscription.id} className="bg-white rounded-xl p-6 border border-yfc-cream-200 shadow-md relative z-10">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-bold text-gray-800 font-serif">{subscription.name}</h2>
                ...
                <ul className="space-y-1">
                  {subscription.features.map((feature, index) => (
                    <li key={index} className="text-gray-800 flex items-center">
                      <svg className="h-4 w-4 text-yfc-gold-500 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
