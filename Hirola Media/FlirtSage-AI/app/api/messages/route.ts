...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (userContentKeywords.includes('musulmane') || userContentKeywords.includes('musulman') || 
          userContentKeywords.includes('islam') || userContentKeywords.includes('religion')) {
        fallbackMessage += `Pour les rencontres avec des personnes de différentes cultures ou religions, je suggère de toujours commencer par le respect mutuel et la compréhension. Chaque personne est unique, au-delà de sa culture ou religion. Je te conseille de l'aborder comme n'importe quelle autre personne qui t'intéresse, tout en étant respectueux de ses valeurs et croyances.\n\n`;
      } else if (userContentKeywords.includes('femme') || userContentKeywords.includes('fille') || userContentKeywords.includes('relations')) {
