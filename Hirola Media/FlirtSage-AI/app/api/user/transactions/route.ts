...
export async function GET(request: Request) {
  try {
    ...
    const enrichedTransactions = paginatedTransactions.map(transaction => {
      // Déterminer la catégorie de la transaction
      let category = transaction.type === 'CREDIT' ? 'Recharge tokens' : 'Utilisation tokens';
      
      // Ajouter des détails en fonction du type de contenu si disponible
      if (transaction.contentType) {
        if (transaction.contentType === 'MESSAGE') {
          category = 'Message envoyé';
        } else if (transaction.contentType === 'COACHING') {
          category = 'Session de coaching';
        }
      }
